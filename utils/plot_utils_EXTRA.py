...
def get_AL_psths(sim, Spikes, wl=100):
    ...
    for glom in hemi_gloms:
        glom_orn_pos = df_char_ids[(df_char_ids.altype=='ORN') & (df_char_ids.glom==glom)].index.values
        glom_pn_pos = df_char_ids[(df_char_ids.altype=='PN') & (df_char_ids.glom==glom)].index.values

        orn_psths.append(get_PSTH_from_spike_array(Spikes[glom_orn_pos, :], dt=sim.dt, wl=wl))
        pn_psths.append(get_PSTH_from_spike_array(Spikes[glom_pn_pos, :], dt=sim.dt, wl=wl))
    orn_psths = np.array(orn_psths); pn_psths = np.array(pn_psths)
    ...
    return orn_psths, pn_psths, eln_psths, iln_psths
...
def plot_sim_spikes(sim, Spikes, show_ieLNs=True, rordering=[], compress_ORNs=0, msize=3):    
    '''
    Given an instance of Simulation class (sim),
    and neurons x timepoints table of spikes (Spikes),
    plots rasters for ORNs, LNs, PNs, and overlays odor signal
    '''
    
    df_char_ids = sim.df_char_ids.copy()
    if len(rordering) == 0:
        rordering = np.arange(len(sim.neur_names))
        
    Spikes = Spikes[rordering, :]
    ...
    orn_gloms = df_char_ids.iloc[rordering][df_char_ids.altype=='ORN']['glom']
    ...
    for i in range(len(glom_changepts)-1):
        ...
        plt.fill_between([0,1],y1=y1, y2=y2)
        plt.text(0, (y1+y2)/2, orn_gloms.iloc[g1], ha='right', va='center', fontsize=6)
        
    # reset color ordering    
    plt.gca().set_prop_cycle(None)
    
    pn_gloms = df_char_ids.iloc[rordering][df_char_ids.altype=='PN']['glom']
    glom_changepts = np.where(~pn_gloms.duplicated())[0]
    for i in range(len(glom_changepts)-1):
        g1, g2 = glom_changepts[i:i+2]
        y1, y2 = sim.nAL-sim.nORNs-sim.nLNs-g2, sim.nAL-sim.nORNs-sim.nLNs-g1
        plt.fill_between([0,1],y1=y1, y2=y2)
        plt.text(0, (y1+y2)/2, pn_gloms.iloc[g1], ha='right', va='center', fontsize=1)
        
        
    # label iLNs
    ln_types = pd.Series(sim.neur_types[rordering])[sim.LNpos]
    for i in range(len(ln_types)):
        g1 = ln_types.index[i]; g2 = g1 + 1
        c = 'blue' if ln_types.iloc[i] == 'iLN' else 'red'
        plt.fill_between([0,1],y1=sim.nAL-g2, y2=sim.nAL-g1, color=c)
        
    if show_ieLNs:
        #ax2.axhline(sim.nAL - sim.nORNs - sim.niLNs + 0.5, ls='--', color='0.6')
        plt.text(0, sim.nAL - np.mean(sim.eLNpos), 'eLNs', color='red', ha='right')
        plt.text(0, sim.nAL - np.mean(sim.iLNpos), 'iLNs', color='blue', ha='right')
        
    # label + plot dividing lines between ORNs, LNs, PNs
    #ax2.axhline(sim.nAL - sim.nORNs + 0.5, ls='--', color='0.6')
    #ax2.axhline(sim.nAL - sim.nORNs - sim.nLNs + 0.5, ls='--', color='0.6')
    yticks = [sim.nAL - np.mean(sim.PNpos), 
              sim.nAL - np.mean(sim.LNpos), 
              sim.nAL - np.mean(sim.ORNpos)]
    yticklabels = ['PNs', 'LNs', 'ORNs']
        
    for i in range(len(yticks)):
        plt.text(-1, yticks[i], yticklabels[i], size=18, rotation=90)
    
    #plt.yticks(yticks, yticklabels, size=14)
...
def plot_sim_spikes2(sim, Spikes, df_AL_activity, subsampling=15, msize=3):    
    ...
    df_pns = sim.df_char_ids.copy()[sim.df_char_ids.altype == 'PN']
    df_pns['glom'] = pd.Categorical(df_pns['glom'], categories=gloms_in_orn_act_order, ordered=True)
    df_pns = df_pns.sort_values(['altype2', 'glom'], ascending=[0,1])
    ...
    df_char_ids = sim.df_char_ids.copy()
    df_char_ids.loc[sim.LNpos, 'polarity'] = -1
    df_char_ids.loc[sim.eLNpos, 'polarity'] = +1
    df_char_ids = df_char_ids.iloc[rordering].reset_index()
    df_char_ids['text_lab'] = ''
    df_char_ids.loc[~df_char_ids['glom'].duplicated(), 'text_lab'] = df_char_ids.loc[~df_char_ids['glom'].duplicated(), 'glom']
    df_char_ids.loc[df_char_ids.altype == 'LN', 'text_lab'] = ''
    df_char_ids.loc[df_char_ids.altype2 == 'mPN', 'text_lab'] = ''
    df_char_ids['text_lab_pos'] = 0
    df_char_ids.loc[df_char_ids['text_lab'] != '', 'text_lab_pos'] = np.arange(df_char_ids.loc[df_char_ids['text_lab'] != ''].shape[0])
    
    
    df_char_ids['rastype'] = df_char_ids['altype2']
    df_char_ids.loc[(df_char_ids.altype == 'LN') & (df_char_ids.polarity == +1), 'rastype'] = 'eLN'
    df_char_ids.loc[(df_char_ids.altype == 'LN') & (df_char_ids.polarity == -1), 'rastype'] = 'iLN'
    ...
    pn_gloms = df_char_ids.loc[df_char_ids.altype == 'PN', 'glom'].astype(str).unique()
    glom_colors = dict((pn_gloms[i], colors_std[i % len(colors_std)]) for i in range(len(pn_gloms)))
    
    
    ntypes = ['ORN', 'eLN', 'iLN', 'uPN', 'mPN']
    for nt in ntypes:
        pos_nt = df_char_ids[df_char_ids.rastype == nt].index.values
        plt.text(-0.1, N_CELLS - np.mean(pos_nt), nt, ha='right')
    
    # label iLNs
    for i in range(N_CELLS):
        g1 = df_char_ids.index[i]; g2 = g1 + 1
        neur_entry = df_char_ids.iloc[i]
        c = 'k'
...
def plot_AL_activity_dur_pre_odors(df_AL_activity_long):
    ...
    rows = [
        pd.Series(['ORN_ex_pre', 'ORN', 'ORN', 'fr_pre_odor0', 10, False, 0, 'DA1'], index=cols),
        pd.Series(['ORN_ex_dur', 'ORN', 'ORN', 'fr_dur_odor0', 40, True, 0, 'DA1'], index=cols),
        pd.Series(['LN_ex_pre', 'LN', 'LN', 'fr_pre_odor0', 5, False, 0, np.nan], index=cols),
        pd.Series(['LN_ex_dur', 'LN', 'LN', 'fr_dur_odor0', 10, True, 0, np.nan], index=cols),
        pd.Series(['iLN_ex_pre', 'iLN', 'iLN', 'fr_pre_odor0', 5, False, 0, np.nan], index=cols),
        pd.Series(['iLN_ex_dur', 'iLN', 'iLN', 'fr_dur_odor0', 10, True, 0, np.nan], index=cols),
        pd.Series(['eLN_ex_pre', 'eLN', 'eLN', 'fr_pre_odor0', 5, False, 0, np.nan], index=cols),
        pd.Series(['eLN_ex_dur', 'eLN', 'eLN', 'fr_dur_odor0', 10, True, 0, np.nan], index=cols),
        #pd.Series(['PN_ex_pre', 'PN', 'PN', 'fr_pre_odor0', 5, False, 0, 'DA1'], index=cols),
        #pd.Series(['PN_ex_dur', 'PN', 'PN', 'fr_dur_odor0', 90, True, 0, 'DA1'], index=cols),
        pd.Series(['uPN_ex_pre', 'PN', 'uPN', 'fr_pre_odor0', 5, False, 0, 'DA1'], index=cols),
        pd.Series(['uPN_ex_dur', 'PN', 'uPN', 'fr_dur_odor0', 90, True, 0, 'DA1'], index=cols),
        #pd.Series(['mPN_ex_pre', 'PN', 'mPN', 'fr_pre_odor0', 5, False, 0, 'DA1'], index=cols),
        #pd.Series(['mPN_ex_dur', 'PN', 'mPN', 'fr_dur_odor0', 90, True, 0, 'DA1'], index=cols),
    ]
    ...
    show_pts=True
    if show_violin:
        ax = sns.violinplot(x='neur_type2', y='fr', 
                      hue='dur_odor', dodge=True, 
                      palette='husl',
                      alpha=0.1, linewidth=0,
                      data=df_AL_activity_long)
    if show_means:
        ax = sns.pointplot(x='neur_type2', y='fr', 
                      hue='dur_odor', dodge=True, 
                      data=df_AL_activity_long,
    ...
    if show_pts:
        sns.stripplot(x='neur_type2', y='fr', 
                      hue='dur_odor', dodge=True, 
                      color='k', alpha=0.05, 
                      jitter=True,
                      data=df_AL_activity_long)
...
'''    
def plot_PN_ORN_fr_heatmaps(df_orn_frs, df_pn_frs, max_fr=200, oind=[], pind=[], annot=False):
    
    
    if len(oind) == len(df_orn_frs):
        df_orn_frs.index = oind
    if len(pind) == len(df_pn_frs):
        df_pn_frs.index = pind  
        
    #plt.figure(figsize=(7,4))
    plt.subplot(121)
    plt.title('ORN fr (Hz)')
    sns.heatmap(df_orn_frs, annot=annot, fmt='.0f', cmap='viridis', vmin=0, vmax=max_fr*1.05)
    plt.xticks(rotation=30, ha='right')

    plt.subplot(122)
    plt.title('PN fr (Hz)')
    sns.heatmap(df_pn_frs, annot=annot, fmt='.0f',cmap='viridis', vmin=0, vmax=max_fr*1.05)
    #plt.yticks([])
    plt.xticks(rotation=30, ha='right')
...
def plot_PN_ORN_fr_heatmaps(df_orn_frs, df_pn_frs, df_char_ids, max_fr=200, saveto_dir='', showplot=False):
    ...
    pn_gloms = df_char_ids[df_char_ids.altype=='PN']['glom']
    glom_changepts = np.where(~pn_gloms.duplicated())[0]
...
def plot_PN_ORN_histograms(df_orn_frs, df_pn_frs, max_fr=200, oind=[], pind=[]):
    ...
    plt.subplot(121)
    plt.title('ORN frs across odors/gloms')
    plt.hist(df_orn_frs.values.flatten(), bins=b, label='ORN', color='xkcd:light blue')
    plt.xlabel('ORN firing rate (Hz)')
    plt.ylabel('odor/glomerulus pair')
    plt.xlim(0, max_fr*1.05)

    plt.subplot(122)
    plt.title('PN frs across odors/gloms')
    plt.hist(df_pn_frs.values.flatten(), bins=b, label='PN', color='xkcd:navy')
    plt.xlabel('PN firing rate (Hz)')
    plt.xlim(0, max_fr*1.05)
...
def plot_orn_ln_pn_scaled_conmat(fig, df_char_ids, final_AL_block):
    ...
    final_orn_pos = np.where(df_char_ids.altype == 'ORN')[0]
    final_ln_pos = np.where(df_char_ids.altype == 'LN')[0]
    final_upn_pos = np.where(df_char_ids.altype2 == 'uPN')[0]
    final_mpn_pos = np.where(df_char_ids.altype2 == 'mPN')[0]
